                ...
                <div class="row breadcrumbs-top">
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active"><a href="{{route('settings.product.sub-category')}}">Product Sub-Category List</a>
                            </li>
                        </ol>
                    </div>
                </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Product Sub-Category Insert/Update Form</h4>
                                            ...
                                            <div class="row">
                                                <div class="col-md-4 no-padding">
                                                    <div class="form-group">
                                                        ...
                                                        <select id="ProductCategory" class="select2 form-control" name="product_category" required>
                                                ...
                                                <div class="col-md-8 no-padding">
                                                    <div class="form-group">
                                                        <label for="ProductSubCategory" class="">Product Sub Category Name</label>
                                                        <input type="text" id="ProductSubCategory" maxlength="100" class="form-control" placeholder="Enter Product Sub-Category Name" name="name" required>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div class="card-body card-dashboard">
                                    <table id="social-media-table" class="table table-striped table-bordered table-condensed social-media table-info">
                                        ...
                                        @if(!empty($manufacturers))
                                            @foreach($manufacturers as $media)
...
@section('pageScripts')
    ...
    <script>
        ...
        $('#social-media-table').on('click',".DeleteWorkExp", function(){
            ...
            var url = '{{ route('settings.product.sub-category.delete') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#social-media-table').on('click',".DeActivateWorkExp", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('settings.product.sub-category.de-activate') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#social-media-table').on('click',".ActivateWorkExp", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('settings.product.sub-category.activate') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#social-media-table').on('click',".EditWorkExp", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('settings.product.sub-category.edit') }}';
            $.ajax({
                url: url,
                method:'POST',
                data:{id: id, _token: '{{csrf_token()}}'},
